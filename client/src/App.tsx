...
import { GedepAccessProvider } from "@/contexts/GedepAccessContext";
...
function AppGate() {
  ...
  useEffect(() => { if (previousStatus.current !== auth.status && auth.status === "signed_in") access.record({ action: "login", area: "Sessão", outcome: "permitido", detail: "Login Google concluído." }); if (previousStatus.current === "signed_in" && auth.status === "signed_out") access.record({ action: "logout", area: "Sessão", outcome: "permitido", detail: "Sessão encerrada pelo usuário." }); previousStatus.current = auth.status; }, [auth.status, access]);
...
export default function App() {
  return <GedepAuthProvider><GedepAccessProvider><GedepStateProvider><AppGate /><Toaster position="top-right" richColors /></GedepStateProvider></GedepAccessProvider></GedepAuthProvider>;
}
